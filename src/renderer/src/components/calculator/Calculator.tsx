...
const CustomTabPanel = (props: TTabPanelProps): JSX.Element => {
...
export const Calculator = (): JSX.Element => {
  ...
  const handleExpressionFieldCaretChange = (e: any): void => {
    setCaretStart((e.target as HTMLInputElement).selectionStart ?? 0);
  };

  const handleCalcButtonAction = (action: E_CALCULATOR_BUTTONS): void => {
  ...
  const handleExpressionCopy = async (e: SyntheticEvent): Promise<void> => {
    e.preventDefault();
    await navigator.clipboard.writeText(expression);
    toast.success('Expression copied to clipboard');
  };

  const handleExpressionPaste = async (e: SyntheticEvent): Promise<void> => {
    e.preventDefault();
  ...
  const BasicCalculatorButton =
    ...
    ({ value, mode, color }: TCalculatorButton): JSX.Element => (
      <CalculatorButton
        key={value}
        value={value}
        isSquare={isSquare}
